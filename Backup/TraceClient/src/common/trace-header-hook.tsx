...
// import { StyledHeader, StyledLinkButton, StyledMenuLinkButton, StyledLogoName } from './trace-styled-components'
...
function useTraceHeader({ setRefresh }: any) {
    ...
    const { setLoginData, getLoginData }: any = manageEntitiesState() //login data is independent of any entity
    ...
    const { resetForm, clearServerError, getValidationFabric
        , getFormData, showServerError, releaseForm } = manageFormsState()
    ...
    function setDisplayArray() {
        ...
        if (getLoginData().userType === 's') { // super admin
            displayArray = ['superAdmin', 'logout']
        } else if (getLoginData().userType === 'a') { // admin            
    ...
    function getDialogTitle() {
        return <DialogTitle disableTypography id="simple-dialog-title"
    ...
    function getDisplayList() {
        const listItemObject: any = {
            superAdmin: <ListItem button key={0}
                onClick={e => {
            ...
            admin: <ListItem button key={1} onClick={e => {
                meta.current.showDialog = false
            ...
            changeUid: <ListItem button key={2} onClick={e => {
            ...
            changePwd: <ListItem button key={3} onClick={e => {
            ...
            logout: <ListItem button key={4} onClick={e => {
                logout()
                meta.current.showDialog = false
                meta.current.isMounted && setRefresh({})
            }}>
        ...
        const listItemArray: any[] = meta.current.dialogConfig.displayArray.map((item: string) => listItemObject[item])
    ...
    function doLogin() {
        ...
        meta.current.dialogConfig.dialogContent =
            <DialogContent>
                <Comp></Comp>
                <Box component='div' className={classes.forgotPwdBox}><Button
                    size='small'
                    variant='text'
    ...
    async function submitDialog() {
        ...
        async function processLogin(data: any) {
            ...
            if (token) {
                ...
                showServerError(meta.current.dialogConfig?.formId, messages['invalidUidOrPassword'])
            }
        }
        ...
        async function processChangeUid(data: any) {
            ...
            const q = queries['genericUpdateMaster'](sqlObjectString, entityName)
            if (q) {
                try {
                    const result = await mutateGraphql(q)
                    if (result?.data?.authentication?.genericUpdateMaster === true) {
                        logout()
                        closeDialog()
                    }
                } catch (exception) {
                    showServerError(meta.current.dialogConfig.formId, exception.message)
                    meta.current.isMounted && setRefresh({})
                }
            }
        }
            ...
            if (q) {
                try {
                    ...
                    if (result?.data?.authentication?.changePwd) {
                        ...
                        showServerError(meta.current.dialogConfig.formId, messages['changePwdFail'])
                        meta.current.isMounted && setRefresh({})
                    }
                } catch (exception) {
                    showServerError(meta.current.dialogConfig.formId, exception.message)
        ...
        if (isValidForm(formId)) {
            ...
            try {
                ...
                emit('SHOW-MESSAGE', { severity: 'error', message: error.message, duration: null })
            }
            // meta.current.dialogConfig.showSpinner = 'none'
            // setRefresh({})

        } else {
            showServerError(meta.current.dialogConfig.formId, globalMessages['validationError'])
        }
    }
    ...
    function logout() {
        ...
        emit('LOAD-MAIN-JUST-REFRESH', 'reset')// reset value resets the componentName in the trace-left.tsx file. So next time when top menu item is clicked the earlier component is not loaded again
    }
    ...
    , "items": [
        ...
        , {
            ...
            , "validations": [
                ...
                //, {
                //     "name": "alphanumericWithSpecialChars",
                //     "message": "Password must have lowercase, uppercase, number, special character and it must be of size 8 or more"
                // }
            ]
        }
    ]
    ...
    "children": [
        {
            ...
            "children": [
                ...
                },
                {
                    ...
                    "componentName": "manageClients"
                }
                , {
                    "name": "manageEntities",
                    "label": "Manage entities",
                    "componentName": "manageEntities"
                }
                , {
                    "name": "allocateEntitiesToClients",
                    "label": "Allocate entities to clients",
                    "componentName": "allocateEntitiesToClients"
                }
            ]
        }
    ]
    ...
    "children": [
        {
            ...
            "children": [
                ...
                , {
                    "name": "manageBu",
                    "label": "Create business unit (Bu)",
                    "componentName": "manageBu"
                }
                , {
                    "name": "allocateUsersToEntities",
                    "label": "Allocate users to entities",
                    "componentName": "allocateUsersToEntities"
                }
            ]
        }
    ]
        ...
        , {
            ...
            , "validations": [
                ...
                },
                {
                    ...
                    "message": "Special characters and white spaces are not allowed in uid"
                }
            ]
        }
    ]
...
// function getDisplayValue(item: string) {
//     let ret: string = 'none'
//     if (meta.current.dialogConfig.displayArray.includes(item)) {
//         ret = 'block'
//     }
//     return ret
// }
