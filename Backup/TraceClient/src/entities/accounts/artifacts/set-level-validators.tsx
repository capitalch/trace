...
import messages from '../../accounts/accounts-messages.json'
const setLevelValidators: any = {
    gstPaymentVoucherValidation: (a: any, dataObject: any, putErrors: any) => {
        ...
        if (dataObject.gst && dataObject.gst.isGst) {
            ...
            const backCalculatedGst:number = Number(((amount * rate) / (rate + 100)).toFixed(2))
            if (igst === 0) {
                if (Math.abs(sgst + cgst - backCalculatedGst) <= 0.9) { //permissible limit
                    ret = undefined
                }
            } else {
