...
def execSql(dbName, sqlString, args=None, isMultipleRows=True,  autoCommitMode=False, buCode='public'):
    out = None
    connection = None
    searchPathSql = getschemaSearchPath(buCode)  #'' if buCode=='public' else f'set search_path to {buCode}'
    try:
        connection, cursor, pool = getConnectionCursor(dbName, autoCommitMode)
...
def execGenericUpdateMaster(dbName, sqlObject, buCode='public'):
    ...
    try:
        ...
        connection = pool.getconn()
        cursor = connection.cursor()
        execSqlObject(sqlObject, cursor, buCode=buCode)
        connection.commit()
        ret = True
