const builtinValidators: any = {
    ...
    alphanumericWithSpecialChars: (a: any, value: string, putErrors: any) => {
        ...
        let ret: any =
            message ||
            a.message ||
            'Must have alphanumeric with at least one special character'
        const isValid: boolean = /^(?=.*?[0-9])(?=.*?[A-Z])(?=.*?[#?!@$%^&*\-_]).{8,}$/.test(
            value
        )
    ...
    emailOrNoWhiteSpaceOrSpecialChar: (
        a: any,
        value: string,
        putErrors: any
    ) => {
    ...
    maxLength: (a: any, value: string, putErrors: any) => {
        const message =
            ...
            a.args.length > 0 &&
            a.message &&
            a.message.replace('$args', a.args[0].toString())
        const len = a.args[0]
        let ret: any =
            message ||
            a.message ||
            'Maximum length can be'.concat(' ', len.toString())
    ...
    minLength: (a: any, value: string, putErrors: any) => {
        ...
        let ret: any =
            message ||
            a.message ||
            'Minimum length required is'.concat(' ', len.toString())
        ;(!value || value.length >= len) && (ret = undefined)
        putErrors(a.name, ret)
    },
    ...
    noSpecialChar: (a: any, value: string, putErrors: any) => {
        const message =
            ...
            a.message.replace('$args', a.args[0])
        let ret: any =
            message || a.message || 'Special characters are not allowed'
    ...
    numbersOnly: (a: any, value: string, putErrors: any) => {
        const message =
            ...
            a.message.replace('$args', a.args[0])
        let ret: any =
            message || a.message || 'Please only enter numeric characters'
    ...
    pinCodeIndia: (a: any, value: string, putErrors: any) => {
        const message =
            a.args &&
            Array.isArray(a.args) &&
            a.args.length > 0 &&
            a.message &&
            a.message.replace('$args', a.args[0])
        let ret: any = message || a.message || 'Invalid pin code'
        const isValid: boolean = /^[1-9]{1}[0-9]{2}\s{0,1}[0-9]{3}$/.test(value)
        isValid && (ret = undefined)
        putErrors(a.name, ret)
    },
    ...
    password: (a: any, value: string, putErrors: any) => {
        ...
        let ret: any =
            message ||
            a.message ||
            'Password must be more than 8 character. Should contain atleast one upper case, one lower case, one digit and one special character'
        !(
            value &&
            !/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/.test(
                value
    ...
    required: (a: any, value: any, putErrors: any) => {
        ...
        value &&
            (String(value).length > 0 || value === true) &&
            (ret = undefined)
        putErrors(a.name, ret)
    },
    ...
    shouldNotContainSpecificChar: (a: any, value: string, putErrors: any) => {
        ...
        let ret: any =
            message ||
            a.message ||
            'Should not have '.concat(a.args[0], ' in the value')
