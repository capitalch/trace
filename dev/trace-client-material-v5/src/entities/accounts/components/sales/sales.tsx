...
function Sales({ saleType }: any) {
    const classes = useStyles()
    const { arbitraryData, handleChange, meta } = useSales(saleType)
    ...
    return (
        <div className={classes.content}>
            <SaleCrown arbitraryData={arbitraryData.current} saleType={saleType} />
            <Tabs
                className="tabs"
            ...
            <div hidden={meta.current.tabValue !== 3}>
                <SaleView arbitraryData={arbitraryData.current} />
            </div>
        </div>
    )
}
