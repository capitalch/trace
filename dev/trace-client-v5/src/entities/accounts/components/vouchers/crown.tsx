...
function Crown({ meta }: any) {
    ...
    const { emit, filterOn, Paper, Typography } = useSharedElements()
...
function Crown1({ meta }: any) {
    ...
    const { filterOn, Paper, Typography } = useSharedElements()
    const { ResetButton, SubmitButton, SummaryDebitsCredits, SummaryGst } =
        useCrown(meta)
    useEffect(() => {
