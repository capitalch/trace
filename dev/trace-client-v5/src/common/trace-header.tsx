...
import ArrowDropDown from '@mui/icons-material/ArrowDropDownSharp'
import {
...
function TraceHeader({ open, handleDrawerOpen }: any) {
    ...
    const { getLoginData, setLoginData, setCurrentEntity, resetBag }: any = manageEntitiesState() //login data is independent of any entity
                ...
                <Button key='1' className={clsx(classes.appBarButton, classes.loginButton)}
                    ...
                    {meta.current.uid ?
                        <Chip
                            ...
                            color='secondary'
                            clickable={true}
                            label={<Typography variant='body2'>{'Hello '}</Typography>}
                            avatar={<Avatar>{meta.current.uid.substring(0, 2)}</Avatar>}
                        ></Chip>
                        : 'Login'}
                </Button>
            )
