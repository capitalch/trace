...
function useShipTo() {
    ...
    function ShippingDetails() {
        const [, setRefresh] = useState({})
        return (<Box sx={{ display: 'flex', flexDirection: 'column', rowGap: 2.2, justifyContent: 'space-around', p: 2 }}>
            {/* Name */}
            <TextField
            ...
            />
            <Box sx={{ ml: 'auto' }}>
                <Button color='secondary' variant='outlined' size='small' onClick={handleClearShippingAddress} >Clear</Button>
                <Button color='secondary' variant='contained' size='small' sx={{ ml: 2 }} onClick={handleSubmit} >Submit</Button>
            </Box>
        </Box>)
