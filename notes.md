...
with 
    ...
    cte3 as ( -- get otherAccounts for SalesPurchaseTable
        select c2."tranHeaderId", 
        c2."autoRefNo", c1."accountNames"
        from cte2 c2
            join cte1 c1
                on c1."tranHeaderId" = c2."tranHeaderId"                
    ),
    cte4 as ( -- cte1 join cte2 for getting other accounts
        select c2."productId", "productCode", "product", "tranDate", "debits", "credits", "tranType", CONCAT_WS(', ',"accountNames", "remarks") as "remarks"
        ,"timestamp", c2."autoRefNo"
            from cte2 c2
                join cte3 c3
                    on c2."tranHeaderId" = c3."tranHeaderId"
    ) select * from cte3 where "autoRefNo"= 'head\SAL\36\2022'
